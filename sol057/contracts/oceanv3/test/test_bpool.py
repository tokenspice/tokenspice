...
def test_setSwapFee_works():
    pool = _deployBPool()
    pool.setSwapFee(toBase18(0.011), txdict(account0))
...
def test_setSwapFee_fails():
    ...
    with pytest.raises(Exception):
        ...
        pool.setSwapFee(toBase18(0.011), txdict(account1))
    pool.setController(address1, txdict(account0))
    pool.setSwapFee(toBase18(0.011), txdict(account1))  # pass now
...
def test_setController():
    ...
    pool.setController(address0, txdict(account1))
    assert pool.getController() == address0


def test_setPublicSwap():
    pool = _deployBPool()
    pool.setPublicSwap(True, txdict(account0))
    assert pool.isPublicSwap()
    pool.setPublicSwap(False, txdict(account0))
    assert not pool.isPublicSwap()
...
def test_2tokens_basic(T1, T2):
    ...
    T1.approve(pool.address, toBase18(90.0), txdict(account0))
    T2.approve(pool.address, toBase18(10.0), txdict(account0))
    ...
    pool.bind(T1.address, toBase18(90.0), toBase18(9.0), txdict(account0))
    pool.bind(T2.address, toBase18(10.0), toBase18(1.0), txdict(account0))
...
def test_unbind(T1, T2):
    ...
    pool.unbind(T1.address, txdict(account0))
...
def test_public_pool(T1, T2):
    ...
    T1.transfer(address1, toBase18(100.0), txdict(account0))
    T2.transfer(address1, toBase18(100.0), txdict(account0))
    ...
    T1.approve(pool.address, toBase18(100.0), txdict(account1))
    T2.approve(pool.address, toBase18(100.0), txdict(account1))
    poolAmountOut = toBase18(10.0)  # 10 BPT
    maxAmountsIn = [toBase18(100.0), toBase18(100.0)]
    pool.joinPool(poolAmountOut, maxAmountsIn, txdict(account1))
    ...
    pool.exitPool(poolAmountIn, minAmountsOut, txdict(account1))
...
def test_rebind_more_tokens(T1, T2):
    ...
    with pytest.raises(Exception):
        pool.rebind(T1.address, toBase18(120.0), toBase18(9.0), txdict(account0))

    # sufficient allowance
    T1.approve(pool.address, toBase18(30.0), txdict(account0))
    pool.rebind(T1.address, toBase18(120.0), toBase18(9.0), txdict(account0))
...
def test_gulp(T1):
    ...
    T1.approve(pool.address, toBase18(50.0), txdict(account0))
    pool.bind(T1.address, toBase18(2.0), toBase18(50.0), txdict(account0))
    ...
    T1.transfer(pool.address, toBase18(5.0), txdict(account0))
    ...
    pool.gulp(T1.address, txdict(account0))
...
def test_joinSwapExternAmountIn(T1, T2):
    pool = _createPoolWith2Tokens(T1, T2, 90.0, 10.0, 9.0, 1.0)
    T1.approve(pool.address, toBase18(100.0), txdict(account0))
...
def test_joinswapPoolAmountOut(T1, T2):
    ...
    pool.joinswapPoolAmountOut(
        tokenIn_address, poolAmountOut, maxAmountIn, txdict(account0)
    )
...
def test_exitswapPoolAmountIn(T1, T2):
    ...
    pool.exitswapPoolAmountIn(
        tokenOut_address, poolAmountIn, minAmountOut, txdict(account0)
    )
...
def _createPoolWith2Tokens(
    ...
    T1.approve(pool.address, toBase18(bal1), txdict(account0))
    T2.approve(pool.address, toBase18(bal2), txdict(account0))

    assert T1.balanceOf(address0) >= toBase18(bal1)
    assert T2.balanceOf(address0) >= toBase18(bal2)
    pool.bind(T1.address, toBase18(bal1), toBase18(w1), txdict(account0))
    pool.bind(T2.address, toBase18(bal2), toBase18(w2), txdict(account0))
