...
def test_transfer():
    ...
    token.transfer(accounts[1], 1e20, txdict(accounts[0]))
...
def test_transferFrom():
    token = _deployToken()
    token.approve(accounts[1], 6e18, txdict(accounts[0]))
    token.transferFrom(accounts[0], accounts[2], 5e18, txdict(accounts[1]))
...
def _deployToken():
    return BROWNIE_PROJECT057.Simpletoken.deploy(
        "TST", "Test Token", 18, 1e21, txdict(accounts[0])
    )
