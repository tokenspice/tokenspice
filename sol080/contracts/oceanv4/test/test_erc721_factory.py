...
from sol080.contracts.oceanv4.oceanv4util import createDataNFT, create_datatoken_from_dataNFT, \
    create_BPool_from_datatoken,poolAddressFromNewBPoolTx, ROUTER
...
def test_direct():
    ...
    #FIXME: assertions
    # Check initial ocean tokens at the beginning
    OCEANtoken.balanceOf(pool_address) == toBase18(ss_OCEAN_init_liquidity)
...
def test_createBPool_via_util():
    router = ROUTER()
    router.changeRouterOwner(account0.address, {"from":GOD_ACCOUNT})
    dataNFT = createDataNFT("dataNFT", "DATANFT", account0, router)
    DT = create_datatoken_from_dataNFT("DT", "DTSymbol", 10000, dataNFT, account0)
    pool = create_BPool_from_datatoken(DT, 10, 200, account0)