...
def test_swapExactAmountIn():
    ...
    tokenInOutMarket = [OCEAN.address, datatoken.address, address0]; # [tokenIn,tokenOut,marketFeeAddress]
    ...
    tokenInOutMarket = [datatoken.address, OCEAN.address, address0]; # [tokenIn,tokenOut,marketFeeAddress]
...
def test_joinPool_addTokens():
    ...
    assert tx.events["LOG_JOIN"][0]["tokenAmountIn"] + datatoken.balanceOf(address0) == account0_DT_balance
    assert tx.events["LOG_JOIN"][1]["tokenAmountIn"] + OCEAN.balanceOf(address0) == account0_Ocean_balance
...
def test_joinswapExternAmountIn_addOCEAN():
    ...
    tx = pool.joinswapExternAmountIn(OCEAN.address, oceanAmountIn, minBPTOut, {"from": account0})
    ...
    assert datatoken.balanceOf(sideStaking.address)  == ssContractDTbalance -  tx.events["LOG_JOIN"][1]["tokenAmountIn"]
    assert pool.balanceOf(sideStaking.address) == ssContractBPTbalance + tx.events["LOG_BPT"]["bptAmount"]
...
def test_joinswapPoolAmountOut_addOCEAN():
    ...
    tx = pool.joinswapPoolAmountOut(OCEAN.address, BPTAmountOut, maxOceanIn, {"from": account0})

    assert tx.events["LOG_JOIN"][0]["tokenIn"] == OCEAN.address
    assert tx.events["LOG_JOIN"][1]["tokenIn"] == datatoken.address

    # Check balance
    assert tx.events["LOG_JOIN"][0]["tokenAmountIn"] + OCEAN.balanceOf(address0) == account0_Ocean_balance
    ...
    ssContractDTbalance - tx.events["LOG_JOIN"][1]["tokenAmountIn"] == datatoken.balanceOf(sideStaking.address)
...
def test_exitPool_receiveTokens():
    ...
    assert tx.events["LOG_EXIT"][0]["tokenAmountOut"] + account0_DT_balance == datatoken.balanceOf(address0)
    assert tx.events["LOG_EXIT"][1]["tokenAmountOut"] + account0_Ocean_balance == OCEAN.balanceOf(address0)
...
def test_exitswapPoolAmountIn_receiveOcean():
    ...
    tx = pool.exitswapPoolAmountIn(OCEAN.address, BPTAmountIn, minOceanOut, {"from": account0})
    ...
    assert tx.events["LOG_EXIT"][0]["tokenAmountOut"] + account0_Ocean_balance == OCEAN.balanceOf(address0)
    ...
    assert ssContractDTbalance + tx.events["LOG_EXIT"][1]["tokenAmountOut"] == datatoken.balanceOf(sideStaking.address)
...
def test_exitswapPoolAmountIn_receiveDT():
    ...
    tx =  pool.exitswapPoolAmountIn(datatoken.address, BPTAmountIn, minDTOut, {"from":account0})

    assert OCEAN.balanceOf(address0) == account0_Ocean_balance
    assert pool.balanceOf(address0) == account0_BPT_balance -  tx.events["LOG_BPT"][0]["bptAmount"]
...
def test_exitswapExternAmountOut_receiveOcean():
    ...
    tx = pool.exitswapExternAmountOut(OCEAN.address, maxBPTIn, exactOceanOut, {"from": account0})

    assert datatoken.balanceOf(address0) == account0_DT_balance
    assert pool.balanceOf(address0) == account0_BPT_balance - tx.events["LOG_BPT"][0]["bptAmount"]

    assert tx.events["LOG_EXIT"][0]["tokenAmountOut"] + account0_Ocean_balance == OCEAN.balanceOf(address0)
    assert ssContractBPTbalance - tx.events["LOG_BPT"][0]["bptAmount"] == pool.balanceOf(sideStaking.address)
    assert ssContractDTbalance + tx.events["LOG_EXIT"][1]["tokenAmountOut"] == datatoken.balanceOf(sideStaking.address)
...
def test_exitswapExternAmountOut_receiveDT():
    ...
    tx = pool.exitswapExternAmountOut(datatoken.address, maxBPTIn, exacDTOut, {"from": account0})
